...
from pigeon_core.payload import PigeonTapePayload, TapeFrame, TapeHeaderEntry, TapeHeaderFrame
from pigeon_core.color import normalize_weights, ColorMap, make_palette
from pigeon_core.mapping_job import MappingJob
import pandas as pd
import colorsys
...
logger = get_logger("pigeoncrusher")
...
def append_hue_to_tape_in_place(item, hue: float):
    """Given (idx, PigeonTapePayload), add hue to tape in place."""
    idx, payload = item
...
def append_bin_to_job(item: 'MappingJob', insertion_idx: int, insertion_location: int, shift_hues= True):
    ...
    if insertion_location == 0:
        ...
        if shift_hues:
            item.in_hues = PigeonCrusher.get_hues(len(item.input_bins), item.exclusion)
    elif insertion_location == 1:
        item.output_bins.push(insertion_idx)
        if shift_hues:
            item.out_hues = PigeonCrusher.get_hues(len(item.output_bins), item.exclusion)
...
class PigeonCrusher:
    ...
    def dummy_payloads(
        ...
        while idx < bins * per_bin:
            ...
            yield (bin_idx, PigeonTapePayload(value, origin_bin=bin_idx, tape=initial_tape, bin_path=[bin_idx]))
            idx += 1
            if t_fn:
                import time
                time.sleep(t_fn())
        for bin_idx in range(count % bins):
            initial_tape = [hues[bin_idx]]
            logger.trace(f"TAPE INIT: Dummy payload for bin {bin_idx} created with tape: {initial_tape}")
            yield (bin_idx, PigeonTapePayload(value, origin_bin=bin_idx, tape=[hues[bin_idx]], bin_path=[bin_idx]))
            if t_fn:
                import time
                time.sleep(t_fn())
import logging
...
def plot_bins(bins: List[List[PigeonTapePayload]], title: str, tape_depth: int = 0, tape_mode: str = 'first', complement: bool = False, figsize=(16, 2)):
    avg_hues = []
    for b in bins:
...
from collections import deque, defaultdict
import numpy as np
from pigeon_crushing import MappingJob  # adjust import to your module structure
...
from pigeon_crushing import MappingJob, JobManager, PigeonCrusher, PigeonTapePayload
from pigeon_crusher_helper import PigeonCrushKernelHelper
import pygame
import numpy as np
import colorsys
...
from pigeon_crushing import (
    MappingJob, PigeonTapePayload, PigeonCrusher, JobJoiner
)
...
from collections import deque, defaultdict
from pigeon_crushing import MappingJob, PigeonTapePayload, PigeonCrusher, ColorMap, normalize_weights, JobManager
